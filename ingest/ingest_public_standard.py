...
from rag.qdrant_config import get_client, get_embedder, ensure_named_collection, DB_ERD_REFERENCE_COLLECTION
...
def upsert_payloads(payloads: List[Dict[str, Any]], batch_size: int = 32):
    ...
    for i in tqdm(range(0, len(payloads), batch_size)):
        ...
        client.upsert(collection_name=DB_ERD_REFERENCE_COLLECTION, points=points)

    print(f"[적재 완료] {len(payloads)}개")


def main():
    ensure_named_collection(DB_ERD_REFERENCE_COLLECTION, recreate=False)
